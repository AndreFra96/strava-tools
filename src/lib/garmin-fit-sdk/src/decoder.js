...
class Decoder {
  ...
  checkIntegrity() {
    try {
      ...
      if (
        this.#stream.length <
        fileHeader.headerSize + fileHeader.dataSize + CRC_SIZE
      ) {
        return false;
      }
      ...
      if (
        fileHeader.headerSize === 14 &&
        fileHeader.headerCRC !== 0x0000 &&
      ...
      const fileCRC =
        (buf[fileHeader.headerSize + fileHeader.dataSize + 1] << 8) +
        buf[fileHeader.headerSize + fileHeader.dataSize];
      if (
        fileCRC !=
        CrcCalculator.calculateCRC(
          buf,
          0,
          fileHeader.headerSize + fileHeader.dataSize
        )
      ) {
        return false;
      }
  ...
  read({
    ...
    try {
      if (
        this.#optMergeHeartRates &&
        (!this.#optApplyScaleAndOffset || !this.#optExpandComponents)
      ) {
        this.#throwError(
          "mergeHeartRates requires applyScaleAndOffset and expandComponents to be enabled"
        );
      }
      ...
      if (this.#optMergeHeartRates) {
        HrMesgUtils.mergeHeartRates(
          this.#messages.hrMesgs,
          this.#messages.recordMesgs
        );
      }
    } catch (error) {
      errors.push(error);
  ...
  #decodeNextFile() {
    ...
    while (
      ...
      position + fileHeader.headerSize + fileHeader.dataSize
    ) {
  ...
  #decodeMessageDefinition() {
    ...
    messageDefinition["endianness"] =
      messageDefinition.architecture === 0
    ...
    for (let i = 0; i < messageDefinition.numFields; i++) {
      ...
      fieldDefinition["invalidValue"] =
        FIT.BaseTypeDefinitions[fieldDefinition.baseType].invalid;
      fieldDefinition["baseTypeSize"] =
        FIT.BaseTypeDefinitions[fieldDefinition.baseType].size;
    ...
    if ((recordHeader & DEV_DATA_MASK) === DEV_DATA_MASK) {
      ...
      for (let i = 0; i < numDevFields; i++) {
        ...
        messageDefinition.developerFieldDefinitions.push(
    ...
    let messageProfile =
      Profile.messages[messageDefinition.globalMessageNumber];
    ...
    this.#localMessageDefinitions[messageDefinition.localMesgNum] = {
      ...messageDefinition,
      ...messageProfile,
    };
    ...
    if (
      messageProfile &&
      !this.#messages.hasOwnProperty(messageProfile.messagesKey)
    ) {
  ...
  #decodeMessage() {
    ...
    messageDefinition.fieldDefinitions.forEach((fieldDefinition) => {
      ...
      const { fieldName, rawFieldValue } = this.#readFieldValue(
        messageDefinition,
        fieldDefinition,
        field
      );
      ...
      if (fieldName != null && (field != null || this.#optIncludeUnknownData)) {
        message[fieldName] = {
          rawFieldValue,
          fieldDefinitionNumber: fieldDefinition.fieldDefinitionNumber,
        };
        ...
        if (field.isAccumulated) {
          this.#accumulator.add(
            mesgNum,
            fieldDefinition.fieldDefinitionNumber,
            rawFieldValue
          );
        }
      }
    ...
    messageDefinition.developerFieldDefinitions.forEach(
      (developerFieldDefinition) => {
        ...
        developerFieldDefinition["invalidValue"] =
          FIT.BaseTypeDefinitions[developerFieldDefinition.baseType].invalid;
        developerFieldDefinition["baseTypeSize"] =
          FIT.BaseTypeDefinitions[developerFieldDefinition.baseType].size;

        const { rawFieldValue: fieldValue } = this.#readFieldValue(
          messageDefinition,
          developerFieldDefinition,
          field
        );
    ...
    if (mesgNum === Profile.MesgNum.DEVELOPER_DATA_ID) {
      ...
      const key = Object.keys(this.#developerDataDefinitions).reduce(
        (count, key) =>
          count + this.#developerDataDefinitions[key].fields.length,
        0
      );
  ...
  #readFieldValue(messageDefinition, fieldDefinition, field) {
    const rawFieldValue = this.#readRawFieldValue(
      messageDefinition,
      fieldDefinition,
      field
    );
  ...
  #addDeveloperDataIdToProfile(message) {
    if (
      message == null ||
      message.developerDataIndex.rawFieldValue == null ||
      message.developerDataIndex.rawFieldValue === 0xff
    ) {
      return;
    }
  ...
  #addFieldDescriptionToProfile(message) {
    ...
    if (
      this.#developerDataDefinitions[
        message.developerDataIndex.rawFieldValue
      ] == null
    ) {
      return;
    }
    ...
    try {
      return (
        this.#developerDataDefinitions[
          ...
          (def) =>
            def.fieldDefinitionNumber ==
            developerFieldDefinition.fieldDefinitionNumber
        ) ?? null
      );
    } catch {
      return null;
  ...
  #expandSubFields(mesgNum, message) {
    ...
    this.#fieldsWithSubFields.forEach((name) => {
      const field =
        Profile.messages[mesgNum].fields[message[name].fieldDefinitionNumber];
    ...
    for (let i = 0; i < field.subFields.length; i++) {
      ...
      for (let j = 0; j < subField.map.length; j++) {
        ...
        if (referenceField.rawFieldValue === map.value) {
          message[subField.name] = JSON.parse(
            JSON.stringify(message[field.name])
          );
  ...
  #expandComponents(mesgNum, message, fields) {
    ...
    while (this.#fieldsToExpand.length > 0) {
      ...
      const { rawFieldValue, fieldDefinitionNumber, isSubField } =
        message[name];
      ...
      if (
        UtilsInternal.onlyInvalidValues(
          rawFieldValue,
          FIT.BaseTypeDefinitions[baseType].invalid
        )
      ) {
        continue;
      }
      ...
      for (let j = 0; j < field.components.length; j++) {
        ...
        if (mesg[targetField.name] == null) {
          ...
          const invalidValue =
            baseType != null ? FIT.BaseTypeDefinitions[baseType].invalid : 0xff;
    ...
    Object.keys(mesg).forEach((key) => {
      ...
      mesg[key].rawFieldValue = UtilsInternal.sanitizeValues(
        mesg[key].rawFieldValue
      );
  ...
  #transformValues(message, messageDefinition) {
    ...
    for (const name in message) {
      ...
      if (!isExpandedField) {
        const fieldValue = this.#transformValue(
          messageDefinition,
          field,
          rawFieldValue
        );
  ...
  #transformValue(messageDefinition, field, rawFieldValue) {
    ...
    if (field == null) {
      ...
      fieldValue = this.#applyScaleAndOffset(
        messageDefinition,
        field,
        rawFieldValue
      );
      ...
      fieldValue = this.#convertTypeToString(
        messageDefinition,
        field,
        rawFieldValue
      );
  ...
  #applyScaleAndOffset(messageDefinition, field, rawFieldValue) {
    ...
    if (
      [
        Profile.MesgNum.DEVELOPER_DATA_ID,
        Profile.MesgNum.FIELD_DESCRIPTION,
      ].includes(messageDefinition.globalMessageNumber)
    ) {
      return rawFieldValue;
    }
    ...
    const scale = Array.isArray(field?.scale ?? 1)
      ? field?.scale[0]
      : field?.scale ?? 1;
    const offset = Array.isArray(field?.offset ?? 1)
      ? field?.offset[0]
      : field?.offset ?? 0;
  ...
  #throwError(error = "") {
    throw Error(
      `FIT Runtime Error at byte ${this.#stream.position} ${error}`.trimEnd()
    );
  }
}
